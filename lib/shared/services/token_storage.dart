...
class TokenStorage {
  ...
  // --- TOKEN ---
  static String? getAccessToken() => _box.read<String>(_accessTokenKey);
  static Future<void> saveAccessToken(String token) async => _box.write(_accessTokenKey, token);
  static Future<void> clearAccessToken() async => _box.remove(_accessTokenKey);

  static String? getRefreshToken() => _box.read<String>(_refreshTokenKey);
  static Future<void> saveRefreshToken(String token) async => _box.write(_refreshTokenKey, token);
  static Future<void> clearRefreshToken() async => _box.remove(_refreshTokenKey);

  // --- REMEMBER ME ---
  static bool getRememberMe() => _box.read<bool>(_rememberMeKey) ?? false;
  static Future<void> setRememberMe(bool value) async => _box.write(_rememberMeKey, value);
